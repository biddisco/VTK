...
if( attribute ){
  if(!attribute->GetDsmManager()) attribute->SetDsmManager(this->DsmManager);
  attribute->Update();
...
if((this->GridType & XDMF_GRID_MASK) != XDMF_GRID_UNIFORM){
    ...
    for(i=0; i < this->NumberOfChildren ; i++){
        if(!this->Children[i]->GetDsmManager()) this->Children[i]->SetDsmManager(this->DsmManager);
...
if(!this->Topology->GetDsmManager()) this->Topology->SetDsmManager(this->DsmManager);
if(this->Topology->Update() == XDMF_FAIL){
    XdmfErrorMessage("Error in Update() of Topology");
    return(XDMF_FAIL);
}
if(!this->Geometry->GetDsmManager()) this->Geometry->SetDsmManager(this->DsmManager);
