...
public:
  ...
  double by;
public:
  vtkGaussianPiecewiseFunctionNode(double x_, double h_, double w_, double bx_,
      double by_) :
...
void vtkGaussianPiecewiseFunction::DeepCopy(vtkDataObject *o)
  {
  vtkGaussianPiecewiseFunction *f = vtkGaussianPiecewiseFunction::SafeDownCast(
      o);
...
bool vtkGaussianPiecewiseFunction::UpdateRange()
  ...
  scaleNodesXAxis(
      (this->Range[1] - this->Range[0]) / (oldRange[1] - oldRange[0]));
...
void vtkGaussianPiecewiseFunction::scaleAndShift(double oldRange[2],
    double newRange[2])
  {
  double a = oldRange[0];
  double b = oldRange[1];
  ...
  for (int i = 0; i < count; i++)
    {
    this->Internal->Nodes[i]->x = (this->Internal->Nodes[i]->x - a) / (b - a)
        * (d - c) + c;
...
int vtkGaussianPiecewiseFunction::RemoveGaussian(double x)
  ...
  std::vector<vtkGaussianPiecewiseFunctionNode*>::iterator iter = std::find_if(
      this->Internal->Nodes.begin(), this->Internal->Nodes.end(),
...
void vtkGaussianPiecewiseFunction::GetTable(double xStart, double xEnd,
    int size, float* table, int stride)
  {
...
void vtkGaussianPiecewiseFunction::BuildFunctionFromTable(double xStart,
  ...
  for (i = 0; i < size; i++)
    {
    vtkGaussianPiecewiseFunctionNode *node =
        new vtkGaussianPiecewiseFunctionNode;
    node->x = xStart + inc * i;
    node->h = *tptr;
    node->w = inc / 2.0;
    node->bx = 0;
...
//----------------------------------------------------------------------------
vtkGaussianPiecewiseFunction* vtkGaussianPiecewiseFunction::GetData(
    vtkInformation* info)
  { //TBD don't know if this is valid, so don't rely on it
  return
      info ?
          vtkGaussianPiecewiseFunction::SafeDownCast(info->Get(DATA_OBJECT())) :
          0;
  }
