...
int vtkPiecewiseFunction::AddPoint(double x, double y, double midpoint,
	double sharpness)
  {
  // Error check
...
void vtkPiecewiseFunction::AddSegment(double x1, double y1, double x2,
	double y2)
  {
  int done;
...
void vtkPiecewiseFunction::GetTable(double xStart, double xEnd, int size,
...
vtkPiecewiseFunction* vtkPiecewiseFunction::GetData(vtkInformation* info)
  {
  return info ? vtkPiecewiseFunction::SafeDownCast(info->Get(DATA_OBJECT())) : 0;
  }
...
void vtkPiecewiseFunction::PrintSelf(ostream& os, vtkIndent indent)
  ...
  for (i = 0; i < this->Internal->Nodes.size(); i++)
	{
	os << indent << "  " << i << " X: " << this->Internal->Nodes[i]->X << " Y: "
