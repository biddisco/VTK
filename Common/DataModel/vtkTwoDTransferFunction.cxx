...
public:
  ...
  bool operator () ( const vtkTwoDTransferFunctionRegion *Region )
    {
      return ( Region->X < this->X1 ||
               Region->X+Region->Width > this->X2 ||
               Region->Y < this->Y1 ||
			   Region->Y+Region->Height > this->Y2);
    }
};
...
int vtkTwoDTransferFunction::AddRegion( double x, double y, double width, double height, double mode, double max){
...
int vtkTwoDTransferFunction::AddRegion( double x, double y, double width, double height, TransferFnMode mode, double max){
	vtkTwoDTransferFunctionRegion *Region = new vtkTwoDTransferFunctionRegion;
	Region->X =x;
	Region->Y = y;
	Region->Width = width;
...
void vtkTwoDTransferFunction::DeepCopy( vtkDataObject *f ){
 ...
 void vtkTwoDTransferFunction::ShallowCopy( vtkDataObject *f ){
  ...
  void vtkTwoDTransferFunction::GetTable( double x1, double x2, double y1, double y2, int sizeX, int sizeY, float *table, int strideX, int strideY ){


  }

  void vtkTwoDTransferFunction::GetTable( double x1, double x2, double y1, double y2, int sizeX, int sizeY, double *table, int strideX, int strideY ){
	  ...
	  for (int r = 0; r< count; r++){
		  if (this->Internal->Regions[r]->Width <= 0 || this->Internal->Regions[r]->Height <= 0){
		  			  continue; //at best one dimensional. If width or height is negative, something went very wrong
			  }

		  if ( ((this->Internal->Regions[r]->X < x1 && this->Internal->Regions[r]->X + this->Internal->Regions[r]->Width <x1)||
				  (this->Internal->Regions[r]->X > x2 ))||
				  ((this->Internal->Regions[r]->Y < y1 && this->Internal->Regions[r]->Y + this->Internal->Regions[r]->Width <y1)||
				  (this->Internal->Regions[r]->Y > y2)) ){
		  ...
		  if (x1 > this->Internal->Regions[r]->X){
			  ...
			  startX = std::max(int((this->Internal->Regions[r]->X-x1)/(x2-x1)*double(sizeX)),0); //max in case of rounding or numerical
		  }
		  if(x2<this->Internal->Regions[r]->X+this->Internal->Regions[r]->Width){
			  endX = sizeX;
		  }
		  else{
			  endX = std::min(int((this->Internal->Regions[r]->X+this->Internal->Regions[r]->Width-x1)/(x2-x1)*double(sizeX)+0.5),sizeX);
		  ...
		  if (y1 > this->Internal->Regions[r]->Y){
			  ...
			  startY = std::max(int((this->Internal->Regions[r]->Y-y1)/(y2-y1)*double(sizeY)),0);
		  }


		  if(y2<this->Internal->Regions[r]->Y+this->Internal->Regions[r]->Width){
		  			  endY = sizeY;
		  }
		  else{
			  endY = std::min(int((this->Internal->Regions[r]->Y+this->Internal->Regions[r]->Width-y1)/(y2-y1)*double(sizeY)+0.5),sizeX); //min in case of rounding or numerical
		  }
...
void vtkTwoDTransferFunction::scaleAndShift(double oldXRange[2], double oldYRange[2], double newXRange[2], double newYRange[2]){
  ...
  for (int i = 0; i<count; i++){
                  this->Internal->Regions[i]->X =  (this->Internal->Regions[i]->X-ax)*xscale+cx;
                  this->Internal->Regions[i]->Y =  (this->Internal->Regions[i]->Y-ay)*yscale+cy;
                  this->Internal->Regions[i]->Width =  (this->Internal->Regions[i]->Width)*xscale;
                  this->Internal->Regions[i]->Height =  (this->Internal->Regions[i]->Height)*yscale;
          }
  ...
  {//adapt range to Regions
	  ...
	  for (int i = 1; i<this->Internal->Regions.size(); i++){
		  ...
		  maxX = std::max(maxX,this->Internal->Regions[i]->X + this->Internal->Regions[i]->Width);
		  maxY = std::max(maxX,this->Internal->Regions[i]->Y + this->Internal->Regions[i]->Height);
	  }
...
return 0;
  ...
  void vtkTwoDTransferFunction::eliminateOutOfRange(){
	  int done;
		...
		while ( !done )
		  ...
		  std::vector<vtkTwoDTransferFunctionRegion*>::iterator iter =
			std::find_if(this->Internal->Regions.begin(),
							this->Internal->Regions.end(),
  ...
  int vtkTwoDTransferFunction::SetRange(double xmin, double xmax, double ymin, double ymax){

  	  double range[4] = {xmin, xmax, ymin, ymax};
...
double oldXRange[2] = {Range[0],Range[1]};
double oldYRange[2] = {Range[2],Range[3]};
double newXRange[2] = {range[0],range[1]};
double newYRange[2] = {range[2],range[3]};
...
return 1;
  ...
  int vtkTwoDTransferFunction::GetRegionValues( int index, double* val,  TransferFnMode &mo ){
            if (!(index < this->Internal->Regions.size())){
                return -1;
            }
            else{
  ...
  int vtkTwoDTransferFunction::SetRegionValues( int index, double val[5], TransferFnMode mo ){
	  if (!(index < this->Internal->Regions.size())){
		  return -1;
	  }
	  else{
  ...
  int vtkTwoDTransferFunction::SetRegionValue( int index, double val, regionvalue v){
  	  if (!(index < this->Internal->Regions.size()))
  		  return -1;
  ...
  int vtkTwoDTransferFunction::SetRegionMode( int index, TransferFnMode mo){
    	  if (!(index < this->Internal->Regions.size()))
    		  return -1;
    	  else this->Internal->Regions[index]->Mode = mo;
    return index;
    }
