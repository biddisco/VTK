...
vtkCxxSetObjectMacro(vtkFixedPointVolumeRayCastMapper, RayCastImage,
...
void vtkFixedPointVolumeRayCastMapperComputeCS1CGradients(T *dataPtr,
    int dim[3], double spacing[3], double scalarRange[2],
  ...
  for (z = z_start; z < z_limit; z++)
    ...
    if ((z / thread_count) % 8 == 7 && thread_id == 0)
      {
      double args[1];
      args[0] = static_cast<float>(z - z_start)
          / static_cast<float>(z_limit - z_start - 1);
      me->InvokeEvent(vtkCommand::VolumeMapperComputeGradientsProgressEvent,
          args);
      }
    }
...
template<class T>
void vtkFixedPointVolumeRayCastMapperComputeGradients(T *dataPtr, int dim[3],
    double spacing[3], int components, int independent,
    double scalarRange[4][2], unsigned short **gradientNormal,
...
vtkFixedPointVolumeRayCastMapper::vtkFixedPointVolumeRayCastMapper()
  ...
  this->CompositeSCGOHelper =
      vtkFixedPointVolumeRayCastCompositeSCGOHelper::New();
  this->CompositeShadeHelper =
      vtkFixedPointVolumeRayCastCompositeShadeHelper::New();
  this->CompositeGOShadeHelper =
      vtkFixedPointVolumeRayCastCompositeGOShadeHelper::New();
  this->CompositeSCGOShadeHelper =
      vtkFixedPointVolumeRayCastCompositeSCGOShadeHelper::New();
...
float vtkFixedPointVolumeRayCastMapper::ComputeRequiredImageSampleDistance(
  ...
  if (oldTime == 0.0)
    ...
    result =
        (result > this->MaximumImageSampleDistance) ?
...
void vtkFixedPointVolumeRayCastMapper::UpdateMinMaxVolume(vtkVolume *vol)
  ...
  this->SpaceLeapFilter->SetComputeGradientOpacity(
      (needToUpdate & 0x04) ? 1 : 0);
  ...
  for (unsigned int compIdx = 0; compIdx < 4; ++compIdx)
    {
    this->SpaceLeapFilter->SetScalarOpacityTable(compIdx,
        this->ScalarOpacityTable[compIdx]);
    this->SpaceLeapFilter->SetGradientOpacityTable(compIdx,
        this->GradientOpacityTable[compIdx]);
    }
  this->SpaceLeapFilter->SetCache(this->MinMaxVolumeCache);
  this->SpaceLeapFilter->Update();
  this->MinMaxVolume = this->SpaceLeapFilter->GetMinMaxVolume(
      this->MinMaxVolumeSize);
...
void vtkFixedPointVolumeRayCastMapper::UpdateCroppingRegions()
  ...
  for (i = 0; i < 6; i++)
    {
    this->FixedPointCroppingRegionPlanes[i] = this->ToFixedPointPosition(
...
int vtkFixedPointVolumeRayCastMapper::PerImageInitialization(vtkRenderer *ren,
    vtkVolume *vol, int multiRender, double inputOrigin[3],
    double inputSpacing[3], int inputExtent[6])
  {
  ...
  if (this->AutoAdjustSampleDistances)
    {
    this->ImageSampleDistance = this->ComputeRequiredImageSampleDistance(
        vol->GetAllocatedRenderTime(), ren, vol);
...
void vtkFixedPointVolumeRayCastMapper::PerVolumeInitialization(vtkRenderer *ren,
    vtkVolume *vol)
  {
  ...
  int usingCellColors;
  this->CurrentScalars = this->GetScalars(input, this->ScalarMode,
  ...
  if (this->LockSampleDistanceToInputSpacing)
    ...
    double avgNumVoxels = pow(
        static_cast<double>((inputExtent[1] - inputExtent[0])
            * (inputExtent[3] - inputExtent[2])
            * (inputExtent[5] - inputExtent[4])), static_cast<double>(0.333));
...
void vtkFixedPointVolumeRayCastMapper::CaptureZBuffer(vtkRenderer *ren)
  ...
  if (this->IntermixIntersectingGeometry && ren->GetNumberOfPropsRendered())
    ...
    zbufferSize[0] = static_cast<int>(static_cast<float>(imageInUseSize[0])
        * this->ImageSampleDistance);
    zbufferSize[1] = static_cast<int>(static_cast<float>(imageInUseSize[1])
        * this->ImageSampleDistance);
    ...
    zbufferOrigin[0] = static_cast<int>(static_cast<float>(imageOrigin[0])
        * this->ImageSampleDistance);
    zbufferOrigin[1] = static_cast<int>(static_cast<float>(imageOrigin[1])
        * this->ImageSampleDistance);
...
void vtkFixedPointVolumeRayCastMapper::Render(vtkRenderer *ren, vtkVolume *vol)
  ...
  if (this->Threader->GetNumberOfThreads() > 1 && this->ThreadWarning)
    {
    vtkWarningMacro(
        "Number of threads " << this->Threader->GetNumberOfThreads() << " is  > 1. This class does not produce repeatable results when the number of threads exceeds 1.");
  ...
  double dummyOrigin[3] =
    {
    0.0, 0.0, 0.0
    };
  double dummySpacing[3] =
    {
    0.0, 0.0, 0.0
    };
  int dummyExtent[6] =
  ...
  this->PerImageInitialization(ren, vol, 0, dummyOrigin, dummySpacing,
      dummyExtent);
  ...
  this->StoreRenderTime(ren, vol,
      this->TimeToDraw * this->ImageSampleDistance * this->ImageSampleDistance
          * (1.0
              + 0.66 * (this->SampleDistance - this->OldSampleDistance)
...
void vtkFixedPointVolumeRayCastMapper::CreateCanonicalView(vtkVolume *vol,
    vtkImageData *image, int blend_mode, double direction[3], double viewUp[3])
  {
...
void vtkFixedPointVolumeRayCastMapper::ComputeRayInfo(int x, int y,
    unsigned int pos[3], unsigned int dir[3], unsigned int *numSteps)
  {
  float viewRay[3];
  ...
  // center it.
  viewRay[1] = ((static_cast<float>(y) + static_cast<float>(imageOrigin[1]))
  ...
  // -1 to 1 - 2/fullSize. Then add offsetX (which is 1/fullSize) to
  // center it.
  viewRay[0] = ((static_cast<float>(x) + static_cast<float>(imageOrigin[0]))
  ...
  *numSteps = 0;
  ...
  if (this->ClipRayAgainstVolume(rayStart, rayEnd, rayDirection,
      this->CroppingBounds)
      && (this->NumTransformedClippingPlanes == 0
          || this->ClipRayAgainstClippingPlanes(rayStart, rayEnd,
    ...
    double worldLength = vtkMath::Normalize(worldRayDirection)
        / this->SampleDistance;
    ...
    float diff[3];
    diff[0] = (rayStart[0] - originalRayStart[0])
        * ((rayDirection[0] < 0) ? (-1) : (1));
    diff[1] = (rayStart[1] - originalRayStart[1])
        * ((rayDirection[1] < 0) ? (-1) : (1));
    diff[2] = (rayStart[2] - originalRayStart[2])
      ...
      for (stepLoop = 0; stepLoop < 3; stepLoop++)
        ...
        if (dir[stepLoop] & 0x80000000)
          ...
          if (endVal > pos[stepLoop])
            {
            currSteps = static_cast<unsigned int>(1
                + (endVal - pos[stepLoop]) / (dir[stepLoop] & 0x7fffffff));
            }
          else
            {
            currSteps = 0;
...
void vtkFixedPointVolumeRayCastMapper::InitializeRayInfo(vtkVolume *vol)
  ...
  this->CroppingBounds[0] = this->CroppingBounds[2] = this->CroppingBounds[4] =
      0.0;
  ...
  if (this->NumTransformedClippingPlanes > 0)
    ...
    this->TransformedClippingPlanes = new float[4
        * this->NumTransformedClippingPlanes];
    ...
    for (i = 0; i < this->NumTransformedClippingPlanes; i++)
      ...
      vtkPlane *onePlane = (vtkPlane *) this->ClippingPlanes->GetItemAsObject(
          i);
      ...
      float t = sqrt(
          planePtr[0] * planePtr[0] + planePtr[1] * planePtr[1]
      ...
      planePtr[3] = -(planePtr[0] * inputOrigin[0]
          + planePtr[1] * inputOrigin[1] + planePtr[2] * inputOrigin[2]);
      }
    }
  ...
  this->CroppingBounds[0] =
      (this->CroppingBounds[0] < 0) ? (0) : (this->CroppingBounds[0]);
  this->CroppingBounds[0] =
      (this->CroppingBounds[0] > dim[0] - 1) ?
          (dim[0] - 1) : (this->CroppingBounds[0]);
  this->CroppingBounds[1] =
      (this->CroppingBounds[1] < 0) ? (0) : (this->CroppingBounds[1]);
  this->CroppingBounds[1] =
      (this->CroppingBounds[1] > dim[0] - 1) ?
          (dim[0] - 1) : (this->CroppingBounds[1]);
  this->CroppingBounds[2] =
      (this->CroppingBounds[2] < 0) ? (0) : (this->CroppingBounds[2]);
  this->CroppingBounds[2] =
      (this->CroppingBounds[2] > dim[1] - 1) ?
          (dim[1] - 1) : (this->CroppingBounds[2]);
  this->CroppingBounds[3] =
      (this->CroppingBounds[3] < 0) ? (0) : (this->CroppingBounds[3]);
  this->CroppingBounds[3] =
      (this->CroppingBounds[3] > dim[1] - 1) ?
          (dim[1] - 1) : (this->CroppingBounds[3]);
  this->CroppingBounds[4] =
      (this->CroppingBounds[4] < 0) ? (0) : (this->CroppingBounds[4]);
  this->CroppingBounds[4] =
      (this->CroppingBounds[4] > dim[2] - 1) ?
          (dim[2] - 1) : (this->CroppingBounds[4]);
  this->CroppingBounds[5] =
      (this->CroppingBounds[5] < 0) ? (0) : (this->CroppingBounds[5]);
  this->CroppingBounds[5] =
      (this->CroppingBounds[5] > dim[2] - 1) ?
          (dim[2] - 1) : (this->CroppingBounds[5]);
...
int vtkFixedPointVolumeRayCastMapper::ComputeRowBounds(vtkRenderer *ren,
    int imageFlag, int rowBoundsFlag, int inputExtent[6])
  {
  float voxelPoint[3];
  float viewPoint[8][4];
  int i, j, k;
  ...
  if (camPos[0] >= bounds[0] && camPos[0] <= bounds[1] && camPos[1] >= bounds[2]
      && camPos[1] <= bounds[3] && camPos[2] >= bounds[4]
      && camPos[2] <= bounds[5])
    {
    insideFlag = 1;
    }
  ...
  minY = (minY < 0) ? (0) : (minY);
  maxX =
      (maxX > imageViewportSize[0] - 1) ? (imageViewportSize[0] - 1) : (maxX);
  maxY =
      (maxY > imageViewportSize[1] - 1) ? (imageViewportSize[1] - 1) : (maxY);
  ...
  if (insideFlag)
    ...
    for (i = 0; i < 12; i++)
      {
      x1 = (viewPoint[lineIndex[i][0]][0] + 1.0) * 0.5 * imageViewportSize[0]
          - imageOrigin[0];

      y1 = (viewPoint[lineIndex[i][0]][1] + 1.0) * 0.5 * imageViewportSize[1]
          - imageOrigin[1];

      x2 = (viewPoint[lineIndex[i][1]][0] + 1.0) * 0.5 * imageViewportSize[0]
          - imageOrigin[0];

      y2 = (viewPoint[lineIndex[i][1]][1] + 1.0) * 0.5 * imageViewportSize[1]
          - imageOrigin[1];
    ...
    for (j = 0; j < imageInUseSize[1]; j++)
      ...
      for (i = 0; i < 12; i++)
        ...
        if (j >= lines[i][1] && j <= lines[i][3]
          ...
          {
          x1 = lines[i][0]
              + (static_cast<float>(j) - lines[i][1])
                  / (lines[i][3] - lines[i][1]) * (lines[i][2] - lines[i][0]);
          ...
          xlow = (xlow < 0) ? (0) : (xlow);
          xlow =
              (xlow > imageInUseSize[0] - 1) ? (imageInUseSize[0] - 1) : (xlow);

          xhigh = (xhigh < 0) ? (0) : (xhigh);
          xhigh =
              (xhigh > imageInUseSize[0] - 1) ?
  ...
  for (j = 0; j < imageMemorySize[1]; j++)
    {
    if (j % 64 == 1 && this->RenderWindow
        && this->RenderWindow->CheckAbortStatus())
      {
      return 0;
      }
    ...
    if (this->RowBounds[j * 2 + 1] < this->OldRowBounds[j * 2]
      ...
      for (i = 0;
          i <= (this->OldRowBounds[j * 2 + 1] - this->OldRowBounds[j * 2]); i++)
        {
        *(ucptr++) = 0;
        *(ucptr++) = 0;
        *(ucptr++) = 0;
    ...
    // New bounds do overlap with old bounds
      ...
      ucptr = image
          + 4 * (j * imageMemorySize[0] + this->RowBounds[j * 2 + 1] + 1);
      for (i = 0;
          i < (this->OldRowBounds[j * 2 + 1] - this->RowBounds[j * 2 + 1]); i++)
        {
        *(ucptr++) = 0;
        *(ucptr++) = 0;
        *(ucptr++) = 0;
...
void vtkFixedPointVolumeRayCastMapper::ComputeMatrices(double inputOrigin[3],
    double inputSpacing[3], int inputExtent[6], vtkRenderer *ren,
    vtkVolume *vol)
  {
  ...
  this->VoxelsTransform->Translate(extentOrigin[0], extentOrigin[1],
      extentOrigin[2]);

  this->VoxelsTransform->Scale(inputSpacing[0], inputSpacing[1],
      inputSpacing[2]);
...
int vtkFixedPointVolumeRayCastMapper::ClipRayAgainstClippingPlanes(
  ...
  for (i = 0; i < numClippingPlanes; i++)
    ...
    dp = planePtr[0] * rayDir[0] + planePtr[1] * rayDir[1]
        + planePtr[2] * rayDir[2];

    if (dp != 0.0)
      {
      t = -(planePtr[0] * rayStart[0] + planePtr[1] * rayStart[1]
...
int vtkFixedPointVolumeRayCastMapper::ClipRayAgainstVolume(float rayStart[3],
    float rayEnd[3], float rayDirection[3], double bounds[6])
  {
  int loop;
  float diff;
  float t;

  if (rayStart[0] >= bounds[1] || rayStart[1] >= bounds[3]
      || rayStart[2] >= bounds[5] || rayStart[0] < bounds[0]
      || rayStart[1] < bounds[2] || rayStart[2] < bounds[4])
    {
  ...
  if (rayEnd[0] >= bounds[1] || rayEnd[1] >= bounds[3] || rayEnd[2] >= bounds[5]
      || rayEnd[0] < bounds[0] || rayEnd[1] < bounds[2]
      || rayEnd[2] < bounds[4])
    {
    for (loop = 0; loop < 3; loop++)
      {
...
void vtkFixedPointVolumeRayCastMapper::ComputeGradients(vtkVolume *vol)
  ...
  vtkIdType sliceSize = (static_cast<vtkIdType>(dim[0])
      * static_cast<vtkIdType>(dim[1]) * ((independent) ? (components) : (1)));
  ...
  try
    {
    this->ContiguousGradientMagnitude =
        new unsigned char[numSlices * sliceSize];
    }
  catch (...)
    {
    this->ContiguousGradientMagnitude = NULL;
  ...
  if (this->ContiguousGradientMagnitude)
    ...
    for (i = 0; i < numSlices; i++)
      {
      this->GradientMagnitude[i] = this->ContiguousGradientMagnitude
          + i * sliceSize;
      }
    }
  else
  ...
  if (components == 1
    ...
    switch (scalarType)
      {
    vtkTemplateMacro(
        vtkFixedPointVolumeRayCastMapperComputeGradients((VTK_TT * )(dataPtr),
            dim, spacing, components, independent, scalarRange,
            this->GradientNormal, this->GradientMagnitude,
...
int vtkFixedPointVolumeRayCastMapper::UpdateShadingTable(vtkRenderer *ren,
  ...
  for (c = 0;
    ...
    for (i = 0; i < this->DirectionEncoder->GetNumberOfEncodedDirections(); i++)
      {
      *(tablePtr++) = static_cast<unsigned short>((*(rptr++)) * VTKKW_FP_SCALE
          + 0.5);
      *(tablePtr++) = static_cast<unsigned short>((*(gptr++)) * VTKKW_FP_SCALE
          + 0.5);
      *(tablePtr++) = static_cast<unsigned short>((*(bptr++)) * VTKKW_FP_SCALE
          + 0.5);
      }
...
int vtkFixedPointVolumeRayCastMapper::UpdateGradients(vtkVolume *vol)
  ...
  for (int c = 0; c < this->CurrentScalars->GetNumberOfComponents(); c++)
    {
    vtkAbstractPiecewiseFunction *f =
        vol->GetProperty()->GetCurrentGradientOpacity(c);
  ...
  for (int c = 0; c < 4; c++)
    {
    if (this->Volume->GetProperty()->GetTwoDTransferFunction(c)
        && this->TwoDOpacityRequired)
      {
      if (strcmp(
          this->Volume->GetProperty()->GetTwoDTransferFunction(c)->GetType(),
          "Constant"))
        {
        needToUpdate = 1;
        }
...
int vtkFixedPointVolumeRayCastMapper::UpdateColorTable(vtkVolume *vol)
  ...
  for (c = 0;
    ...
    scalarOpacityDistance[c] = vol->GetProperty()->GetScalarOpacityUnitDistance(
        c);
    ...
    if (this->SavedGradientOpacityFunction[c] != gradientOpacityFunc[c]
        || this->SavedParametersMTime.GetMTime()
            < gradientOpacityFunc[c]->GetMTime())
      {
      needToUpdate = 1;
      }
    ...
    if ((this->SavedTwoDTransferFunction[c] != twoDTransferFunc[c]
        || this->SavedParametersMTime.GetMTime()
            < twoDTransferFunc[c]->GetMTime()) && GetTwoDOpacityRequired()) //if we aren't using the twod function, we don't need to update because of it.
      {
      needToUpdate = 1;
      }
    }
  ...
  if (vol->GetProperty()->GetIndependentComponents())
    ...
    for (c = 0; c < components; c++)
      ...
      for (i = 0; i < this->TableSize[c]; i++)
        {
        this->ColorTable[c][3 * i] = static_cast<unsigned short>(tmpArray[3 * i]
            * VTKKW_FP_SCALE + 0.5);
        this->ColorTable[c][3 * i + 1] = static_cast<unsigned short>(tmpArray[3
            * i + 1] * VTKKW_FP_SCALE + 0.5);
        this->ColorTable[c][3 * i + 2] = static_cast<unsigned short>(tmpArray[3
            * i + 2] * VTKKW_FP_SCALE + 0.5);
        }

      if (this->GetTwoDOpacityRequired())
        { //if we use the twodfunc, we need to "fake" the scalartable
        twoDTransferFunc[c]->GetOneDScalarTable(scalarRange[c][0],
            scalarRange[c][1], this->TableSize[c], tmpArray);
        }
      else
        {
      ...
      if (this->BlendMode == vtkVolumeMapper::COMPOSITE_BLEND)
        ...
        double factor;
          {
          factor = this->SampleDistance
              / vol->GetProperty()->GetScalarOpacityUnitDistance(c);
          }
      ...
      for (i = 0; i < this->TableSize[c]; i++)
        {
        this->ScalarOpacityTable[c][i] = static_cast<unsigned short>(tmpArray[i]
            * VTKKW_FP_SCALE + 0.5);
        }
      ...
      if (scalarRange[c][1] - scalarRange[c][0])
        ...
        gradientOpacityFunc[c]->GetTable(0,
            (scalarRange[c][1] - scalarRange[c][0]) * 0.25 / adjustfactor, 256,
            tmpArray);
        for (i = 0; i < 256; i++)
          {
        ...
        if (this->GetTwoDOpacityRequired())
          ...
          twoDTransferFunc[0]->GetTable(scalarRange[c][0], scalarRange[c][1], 0,
              (scalarRange[components - 1][1] - scalarRange[components - 1][0])
                  * 0.25 / adjustfactor, 32768, 256, ttmpArray);
          int size = 32768 * 256;
          for (int i = 0; i < size; i++)
            {
            this->TwoDOpacityTable[c][i] =
                static_cast<unsigned short>(ttmpArray[i] * VTKKW_FP_SCALE + 0.5);
            }
          delete[] ttmpArray;
          }
        }
    ...
    if (components == 2)
      ...
      for (i = 0; i < this->TableSize[0]; i++)
        {
        this->ColorTable[0][3 * i] = static_cast<unsigned short>(tmpArray[3 * i]
            * VTKKW_FP_SCALE + 0.5);
        this->ColorTable[0][3 * i + 1] = static_cast<unsigned short>(tmpArray[3
            * i + 1] * VTKKW_FP_SCALE + 0.5);
        this->ColorTable[0][3 * i + 2] = static_cast<unsigned short>(tmpArray[3
            * i + 2] * VTKKW_FP_SCALE + 0.5);
        }
      }
    ...
    if (this->GetTwoDOpacityRequired())
      ...
      {
      scalarOpacityFunc[0]->GetTable(scalarRange[components - 1][0],
          scalarRange[components - 1][1], this->TableSize[components - 1],
          tmpArray);
      }
    ...
    if (this->BlendMode == vtkVolumeMapper::COMPOSITE_BLEND)
      {
      float *ptr = tmpArray;
      double factor = this->SampleDistance
    ...
    for (i = 0; i < this->TableSize[components - 1]; i++)
      {
      this->ScalarOpacityTable[0][i] = static_cast<unsigned short>(tmpArray[i]
          * VTKKW_FP_SCALE + 0.5);
      }
    ...
    if (scalarRange[components - 1][1] - scalarRange[components - 1][0])
      ...
      if (this->GetTwoDOpacityRequired())
        ...
        twoDTransferFunc[0]->GetOneDGradientTable(0,
            (scalarRange[components - 1][1] - scalarRange[components - 1][0])
                * 0.25 / adjustfactor, 256, tmpArray);
        }
      else
        {
...
//           vol->GetProperty()->GetInterpolationType() == VTK_NEAREST_INTERPOLATION );
  return (vol->GetProperty()->GetInterpolationType()
